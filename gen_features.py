...
.btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;font-size:13px;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;padding:16px 30px;border-radius:10px;border:1px solid transparent;transition:transform .2s,background .2s,box-shadow .2s,border-color .2s,color .2s;}
...
.mega-head{font-size:13.5px;font-weight:600;color:var(--ink);padding-bottom:12px;border-bottom:1px solid #ece5d6;margin-bottom:8px;}
...
.nav-dd-menu a.mega-item.here b{color:var(--gold-deep);}
.mega-item .mi{width:38px;height:38px;border-radius:10px;background:#f7f3ea;border:1px solid #e8e2d4;display:flex;align-items:center;justify-content:center;color:var(--gold-deep);flex-shrink:0;}
.mega-item .mi svg{width:18px;height:18px;stroke-width:1.7;}
.mega-item b{display:block;font-size:13.5px;font-weight:700;color:var(--ink);line-height:1.3;}
...
MEGA_ORDER = ["website", "missed-call-text-back", "review-automation", "lead-follow-up",
              "local-seo", "marketing-campaigns", "crm-and-booking"]
MEGA_LABELS = {"website": "Custom Website", "review-automation": "5-Star Review Funnel"}
MEGA_DESCS = {
    "website": "A lead-generating website, built in days",
    "missed-call-text-back": "Automatically text back missed calls",
    "review-automation": "Get more 5-star reviews on autopilot",
    "lead-follow-up": "Automatically follow up with leads via text",
    "local-seo": "Actually get found on Google",
    "marketing-campaigns": "Keep your customers thinking about you",
    "crm-and-booking": "Every lead and appointment in one place",
}

def features_dd(current_slug):
    by_slug = {p["slug"]: p for p in PAGES}
    items = []
    for slug in MEGA_ORDER:
        p = by_slug[slug]
        here = " here" if slug == current_slug else ""
        items.append(
            '<a href="/features/%s.html" class="mega-item%s">\n'
            '                            <span class="mi">%s</span>\n'
            '                            <div><b>%s</b><span>%s</span></div>\n'
            '                        </a>' % (slug, here, ic(p["icon"]),
                                              esc(MEGA_LABELS.get(slug, p["label"])),
                                              esc(MEGA_DESCS[slug])))
    items.append(
        ...
        '                            <div><b>Everything Included</b><span>See the whole system at a glance</span></div>\n'
        '                        </a>')
    return "\n                        ".join(items)
  ...
  "@graph": [
    ...
    },
    {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "What's included", "item": "https://northernpeaksystems.ca/#included" },
...
<section class="fhero">
    ...
    <div class="wrap">
        <div class="fhero-grid">
            <div>
                <p class="crumbs"><a href="/">Home</a> &nbsp;/&nbsp; <a href="/#included">What&rsquo;s included</a> &nbsp;/&nbsp; %(label)s</p>
