// St. Albert Kinsmen RV & Community Park — shared interactions
(function(){
    ...
    (function(){
        ...
        var triggers=Array.prototype.filter.call(document.querySelectorAll('a[href]'),function(a){
            var href=a.getAttribute('href')||'';
            return imgExt.test(href)&&!a.hasAttribute('data-no-lightbox');
        });
        ...
        function captionFor(a){var cap=a.querySelector('.cap, .scene-cap');if(cap)return cap.textContent.trim();var img=a.querySelector('img');return img&&img.alt?img.alt:''}
        function open(href,alt,caption,trigger){lastFocus=trigger||document.activeElement;imgEl.src=href;imgEl.alt=alt||'';if(caption){capEl.textContent=caption;capEl.style.display=''}else{capEl.textContent='';capEl.style.display='none'}box.classList.add('open');box.setAttribute('aria-hidden','false');document.documentElement.style.overflow='hidden';setTimeout(function(){closeBtn.focus()},50)}
